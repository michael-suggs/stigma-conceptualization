...
GLOVE_TXT: Final[str] = os.path.expandvars('${XDG_DATA_HOME}/michael/data-science/glove.840B.300d.txt')


def sequence_data(X_train, X_test) -> Tuple[np.ndarray, np.ndarray, dict]:
    tokenizer = text.Tokenizer(num_words=None)
    tokenizer.fit_on_texts(list(X_train) + list(X_test))
    X_train_seq = sequence.pad_sequences(tokenizer.texts_to_sequences(X_train), maxlen=1500)
    X_test_seq = sequence.pad_sequences(tokenizer.texts_to_sequences(X_test), maxlen=1500)
...
def lstm_simple(d_inp, d_out, inp_len, ematrix, lstm_units) -> Sequential:
    model = Sequential([
        Embedding(d_inp, d_out, input_length=inp_len,
                  weights=[ematrix], trainable=False),
